<!--
  🌸 WELCOME, CUSTOMIZER! 🌸
  1. Replace ALL [tu-usuario] with your GitHub username.
  2. Replace [tu-portfolio.com] with your portfolio URL (or remove the badge).
  3. If you want to change the colors, just edit the hex codes (#FF69B4 and #3CB371) in the badges and capsule renders.
  4. Feel free to add your own projects in the "Featured Projects" section.
-->

<div align="center">
  <img 
    src="https://capsule-render.vercel.app/api?type=venom&color=0:FFB6C1,100:98FB98&height=200&section=header&text=Mabilia%20Rinelli&fontSize=40&fontColor=454545&animation=fadeIn&fontAlignY=30&desc=Computer%20Science%20Student%20%7C%20Information%20Systems%20Enthusiast&descAlignY=50&descSize=18"
    width="100%"
  />
  
  <img 
    src="https://readme-typing-svg.demolab.com?font=Poppins&weight=500&size=24&pause=1200&color=E8A0B5&center=true&vCenter=true&width=650&lines=Future+Data+Scientist;ETL+%2B+SQL+%3D+%E2%9D%A4%EF%B8%8F;Always+learning+something+new;Born+in+the+Canary+Islands"
    alt="Typing SVG"
  />
  
  <img src="https://komarev.com/ghpvc/?username=marinellipa&style=for-the-badge&color=ffb6c1&label=★+VISITORS" />
</div>


## ⋆✴︎˚｡⋆ About Me (the short version)

Hey there! I'm **Mabilia Rinelli** – a Computer Science student at the **University of Granada (UGR)** with a passion for **Information Systems**. I love turning messy data into clean insights, and I'm always up for a challenge (especially if it involves SQL or ETL pipelines).

- 🎓 **BSc in Computer Science** (2022–2027) – 85% completed.  
- 🏅 Awarded honors in: *Computational Models*, *Multidimensional Databases & Data Warehousing*, *Enterprise Information Systems*, and *Geographic Information Systems*.  
- 📚 Student Delegate and **Equality Committee Coordinator** – because tech should be inclusive.  
- 🌱 Currently obsessed with: **Data Engineering**, **GenAI**, and **Database Optimisation**.  
- 🏝️ Born in Las Palmas de Gran Canaria, now based in Granada (since 2022).  
- 💗 **Fun fact**: I can mediate a conflict as well as I can debug a stored procedure – thanks to my training in peer support and assertiveness!


## 🎓 Education

| Institution | Degree | Period | Highlights |
|-------------|--------|--------|------------|
| **University of Granada (UGR)** | BSc in Computer Science (Information Systems) | 2022 – 2027 | Student Delegate, Equality Coordinator, 4 subject honours |
| **Claret Las Palmas** | Scientific Baccalaureate | 2020 – 2022 | Final grade: 9.7/10; Mediation Program member |


## 💼 Experience & Events

- **NTT Data** – *Risging Software Engineer (Data & AI Department)*  
  Developed ETL processes using **SQL Developer** and **Oracle Data Integrator** – real-world data wrangling!

- **MongoDB Women in Computer Science Summit**  
  Attended workshops on technical interviewing, cloud tech, and database management – met some amazing people!


## 🌿 Tech Stack & Skills

### Languages & Tools
![SQL](https://img.shields.io/badge/SQL-FF69B4?style=for-the-badge&logo=postgresql&logoColor=white)
![C](https://img.shields.io/badge/C-FF69B4?style=for-the-badge&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-FF69B4?style=for-the-badge&logo=cplusplus&logoColor=white)
![LaTeX](https://img.shields.io/badge/LaTeX-FF69B4?style=for-the-badge&logo=latex&logoColor=white)
![Git](https://img.shields.io/badge/Git-3CB371?style=for-the-badge&logo=git&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-3CB371?style=for-the-badge&logo=linux&logoColor=white)
![ODI](https://img.shields.io/badge/Oracle%20Data%20Integrator-FF69B4?style=for-the-badge&logo=oracle&logoColor=white)

### Soft Skills
- Teamwork 🤝
- Problem Solving 🧩
- Adaptability 🌊
- Attention to Detail 🔍

### Languages (I speak)
- **Spanish** – Native 🇪🇸
- **Italian** – Advanced 🇮🇹
- **English** – Proficient 🇬🇧
- **German** – Beginner 🇩🇪

<!--
## ⚘.⋆˚࿔ Featured Projects

| Project | Description | Tech |
|---------|-------------|------|
| **[ETL Pipeline Demo](https://github.com/[tu-usuario]/etl-demo)** | A mini ETL project using Python and SQL to transform sales data. | `Python` `SQL` `Pandas` |
| **[Database Design Lab](https://github.com/[tu-usuario]/db-design)** | ER modelling and normalisation exercises from my coursework. | `SQL` `Diagrams` |
| **[Mediation App](https://github.com/[tu-usuario]/mediation-app)** | A simple tool to help peers practice conflict resolution (just a prototype). | `React` `Node.js` |
-->

## 🌺 My Day in a Nutshell

- 🏐 **Morning**: Gym – gotta stay active!
- 💻 **Late morning**: Classes / coding sessions – databases are my jam.
- 🍝 **Lunch**: Pasta (obviously) + a good podcast.
- 📖 **Afternoon**: Reading – either a novel or a new tech blog or Working.
- 🎧 **Evening**: Chill with music, or dive into a side project.
- 🌙 **Night**: Plan tomorrow’s ETL pipeline (or just dream about it).

## 🌷 Let’s Connect!

I'm always open to chatting about data, tech, or just life. Find me here:

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-FF69B4?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/mabilia-rinelli)
[![Email](https://img.shields.io/badge/Email-3CB371?style=for-the-badge&logo=gmail&logoColor=white)](mailto:rnellimabilia@gmail.com)

</div>

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:98FB98,100:FFB6C1&height=100&section=footer" width="100%"/>
  <br>
</div>
